...
export default function DoctorNav({ userName, specialization, logoutAction }: DoctorNavProps) {
          ...
          return (
            ...
            >
              <item.icon className={`h-5 w-5 transition ${
                active ? "text-secondary" : "text-slate-500 group-hover:text-secondary"
              }`} />
              {item.name}
            </Link>
          );
    ...
    <>
      ...
      <header className="md:hidden flex items-center justify-between px-6 py-4 border-b border-brand-border bg-brand-card/25 backdrop-blur-md sticky top-0 z-40">
        ...
        <div className="flex items-center gap-4">
          <span className="text-[10px] font-semibold px-2 py-1 rounded bg-secondary/10 border border-secondary/20 text-secondary uppercase">
            Doctor
          </span>
          <button
