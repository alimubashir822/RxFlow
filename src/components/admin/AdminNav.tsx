...
export default function AdminNav({ userName, logoutAction }: AdminNavProps) {
          ...
          return (
            ...
            >
              <item.icon className={`h-5 w-5 transition ${
                active ? "text-amber-500" : "text-slate-500 group-hover:text-amber-500"
              }`} />
              {item.name}
            </Link>
          );
    ...
    <>
      ...
      <header className="md:hidden flex items-center justify-between px-6 py-4 border-b border-brand-border bg-brand-card/25 backdrop-blur-md sticky top-0 z-40">
        ...
        <div className="flex items-center gap-4">
          <span className="text-[10px] font-semibold px-2 py-1 rounded bg-amber-500/10 border border-amber-500/20 text-amber-400 uppercase">
            Admin
          </span>
          <button
